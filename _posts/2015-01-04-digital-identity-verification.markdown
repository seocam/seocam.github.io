...
place. Well, unfortuantelly currently there are not a lot of user friendly alternatives but follow
some of them:

* **Time-based**: Relies on if you should be accessing this resource at an specific time. That's not safe
by itself but it can be useful when combined if other authentication methods.
* **Federated Identity**: That's roughly an outsourcing. Instead of trying to check the identity of the user
the Web site will ask to someone elso to check it. This identity verifiers are usually called IdP. 
...
to know everything you are accessing (creepie!).
...
that can make idendetity verification in an accepted level of safety but most the Web Developers don't know or
